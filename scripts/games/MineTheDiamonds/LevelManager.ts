...
export class MineTheDiamondsLevelManager extends GameLevelManager {
  ...
  private spawnNewTargetBlock() {
    system.runTimeout(() => {
      ...
      dimension.runCommandAsync(`setblock ${pos.x} ${pos.y} ${pos.z} ${this.game.currentLevel.blockToBreak}`);
  ...
  private spawnRandomBlocks() {
    ...
    for (let i = 0; i < 20; i++) {
      system.runTimeout(() => {
        const pos = this.getRandomArenaPosition();
        dimension.runCommandAsync(`setblock ${pos.x} ${pos.y} ${pos.z} ${blockType}`);
    ...
    mobs.forEach((mob: { type: string; count: number }) => {
      const interval = system.runInterval(() => {
        for (let i = 0; i < mob.count; i++) {
          const pos = this.getRandomArenaPosition();
          dimension.runCommandAsync(`summon minecraft:${mob.type} ${pos.x} ${pos.y} ${pos.z}`);
        }
      }, 200);
