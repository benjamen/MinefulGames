...
var GameSetup = class {
  ...
  startGame(players, gameArea) {
    ...
    world.sendMessage(`\u{1F3AE} Welcome to ${this.gameName}!`);
  ...
  clearObjectives() {
    world.scoreboard.getObjectives().forEach((objective) => {
      ...
      world.sendMessage(`\u{1F9F9} Removed scoreboard objective: ${objective.id}`);
    });
  }
  ...
  clearArena(gameArea) {
    ...
    try {
      ...
      world.sendMessage(`\u{1F9F9} Cleared the game arena!`);
    } catch (error) {
  ...
  clearPlayerInventories(players) {
    ...
    world.sendMessage("\u{1F9F9} Cleared all players' inventories!");
  }
  ...
  setWorldSettings() {
    ...
    world.getDimension("overworld").runCommand(`difficulty ${this.difficulty}`);
  }
...
var MinecraftDimensionTypes2 = ((MinecraftDimensionTypes22) => {
...
var Utilities = class {
  static fillBlock(blockPerm, xFrom, yFrom, zFrom, xTo, yTo, zTo) {
    const overworld = world2.getDimension(MinecraftDimensionTypes2.Overworld);
...
function Game1(log, location) {
  ...
  const gameSetup = new GameSetup(
    ...
    timerMinutes,
    gameMode,
    // Passing the game mode
    dayOrNight,
    // Passing the time of day (day or night)
    difficulty
    // Passing the difficulty level
  );
  ...
  function initializeBreakTheTerracotta() {
    ...
    let airBlockPerm = BlockPermutation2.resolve(MinecraftBlockTypes.Air);
    let cobblestoneBlockPerm = BlockPermutation2.resolve(MinecraftBlockTypes.Cobblestone);
    if (airBlockPerm) {
      Utilities.fillBlock(
        airBlockPerm,
  ...
  function spawnNewTerracotta() {
    ...
    if (block) {
      block.setPermutation(BlockPermutation2.resolve(MinecraftBlockTypes.YellowGlazedTerracotta));
    }
  }
  function checkForTerracotta() {
    ...
    if (block && !block.permutation.matches(MinecraftBlockTypes.YellowGlazedTerracotta)) {
      ...
      for (const player of players2) {
        ...
        const scoreObjective = world3.scoreboard.getObjective("points");
        if (scoreObjective) {
  ...
  function addFuzzyLeaves() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      overworld.getBlock(Vector3Utils.add(ARENA_VECTOR_OFFSET, { x: leafX, y: leafY, z: leafZ }))?.setPermutation(BlockPermutation2.resolve("minecraft:leaves"));
    }
  }
  system2.run(gameTick);
}
      ...
      if (this.pendingFuncs.length > 0) {
        ...
        if (funcSet) {
          try {
            ...
            world4.sendMessage("Could not run code function. Error: " + e.toString());
          }
        }
      }
    }
    if (this.tickCount === 200) {
      world4.sendMessage("Type '/scriptevent run:<game_name>' in chat to run a specific game.");
    }
    this.tickCount++;
    system3.run(this.worldTick);
  }
