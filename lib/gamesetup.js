...
export class GameSetup {
    ...
    startGame(players, gameArea, arenaoffset, arenaSize) {
        ...
        players.forEach((player) => {
            ...
            console.warn(`Teleporting ${player.name} to: x=${arenaCenter.x}, y=${arenaCenter.y}, z=${arenaCenter.z} in dimension ${gameArea.dimension}`);
    ...
    clearArena(gameArea, arenaSize) {
        ...
        try {
            ...
            dimension.runCommand(`kill @e[type=!player,x=${x - arenaSize.x / 2},y=${y},z=${z - arenaSize.z / 2},dx=${arenaSize.x},dy=${arenaSize.y},dz=${arenaSize.z}]`);
            // Fill the area with air
            dimension.runCommand(`fill ${x - arenaSize.x / 2} ${y} ${z - arenaSize.z / 2} ${x + arenaSize.x / 2} ${y + arenaSize.y} ${z + arenaSize.z / 2} air`);
