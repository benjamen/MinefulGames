...
export class GameSetup {
    ...
    setWorldSettings() {
        ...
        if (this.dayOrNight === "day") {
            world.getDimension("overworld").runCommand("time set day"); // Set to day time
            world.getDimension("overworld").runCommand("gamerule doDaylightCycle true"); // Ensure daylight cycle is enabled
        }
        else {
            world.getDimension("overworld").runCommand("time set night"); // Set to night time
            world.getDimension("overworld").runCommand("gamerule doDaylightCycle false"); // Disable daylight cycle
        }
        // Set the world difficulty
        world.getDimension("overworld").runCommand(`difficulty ${this.difficulty}`); // Set difficulty to the passed parameter
    }
    ...
    endGame(players) {
        ...
        players.forEach((player) => {
            ...
            player.teleport({ x: 0, y: 65, z: 0 }); // Change this to your lobby coordinates
